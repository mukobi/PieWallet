...
<?php
    // add faq code
    if(!empty($_POST) && $_POST['action']=="add_faq"){
        //echo "<pre>"; print_r($_POST);die;
        $stmt = $conn->prepare("INSERT INTO ls_faqs (title, description, created) VALUES (?, ?, ?)");
        $stmt->bind_param("sss", $title, $description, $created);
        $title = $_POST['title'];
    ...
    if(isset($_GET['action']) && $_GET['action']=="delete" && !empty($_GET['id'])){
        $stmt = $conn->prepare("delete from ls_faqs where id = ".$_GET['id']);
        if($stmt->execute()){
            $_SESSION['msg1'] = 'Faq Deleted successfully';
        }
        else{
...
<section class="about-us-wrapper">
	<div class="container" id="login-block">
		...
		<form name="faqForm" action="add-faq.php" method="post">
			...
			<div class="row">
				...
				<div class="col-sm-12 submit-btn">
					<input type="submit" class="btn btn-default" name="add_faq" value="Submit">
				</div>
			</div>
		</form>
	</div>
